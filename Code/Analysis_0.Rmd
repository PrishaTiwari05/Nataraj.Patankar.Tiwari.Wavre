...
# How many states per page, excluding California
...
# Plot California separately with full scale
...
ev_stations <- stations %>%
  ...
  mutate(State = str_trim(toupper(State))) %>%  # Uppercase 2-letter codes and trim whitespace
  group_by(State) %>%
  summarize(num_stations = n()) %>%
  arrange(num_stations) %>%
  mutate(State = factor(State, levels = State))  # factor ordered for plotting
